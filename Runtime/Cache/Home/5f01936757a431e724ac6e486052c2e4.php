...
<div class="container-totton-con">
      ...
      <div class="bottontitle"><span class="bottontitle-span1">网上预约快递</span><span class="bottontitle-span2"><span style="color: red">*为必填信息！</span>为方便快递员取货，请提供您真实有效的信息，并在登陆后下单。<span></div>
      <form class="form-horizontal" method="post" action="<?php echo U('index/manageorder',array('id'=>$info['id']));?>">
      <div class="group">
      <div class="group-title">寄件人信息</div>
  <div class="form-group">
    <label for="name1" class="col-sm-2 control-label"><span class="payattention">*</span>姓名</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="name1" placeholder="姓名" name="sender" value="<?php echo ($info['sender']); ?>">
    </div>
  </div>
  <div class="form-group">
    <label for="tel1" class="col-sm-2 control-label"><span class="payattention">*</span>手机/固话</label>
    <div class="col-sm-10">
      <input type="tel" class="form-control" id="tel1" placeholder="手机号/固话号" name="senderphone" value="<?php echo ($info['sender_phone']); ?>">
    </div>
  </div>
  <div class="form-group">
    <label for="unit1" class="col-sm-2 control-label">寄件单位</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="unit1" placeholder="寄件单位" name="junit" value="<?php echo ($info['sender_company']); ?>">
    </div>
  </div>
   <div class="form-group">
    <label for="remark1" class="col-sm-2 control-label">取件方式备注</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="remark1" placeholder="取件方式有上门取件，自提等" name="remark1" value="<?php echo ($info['fetchgoods_remarks']); ?>">
    </div>
  </div>
      <div class="form-group">
    <label for="remark3" class="col-sm-2 control-label"><span class="payattention">*</span>网点</label>
    <div class="col-sm-10">
      <select class="form-control" name="wangdian">
     <?php if(is_array($wangdian)): $i = 0; $__LIST__ = $wangdian;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$wang): $mod = ($i % 2 );++$i;?><option value="<?php echo ($wang['id']); ?>" <?php if( $info['area_name'] == $wang['area_name']){ echo "selected=selected";} ?>><?php echo ($wang['area_name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
    </div>
  </div>
  ...
  <div class="form-group" style="display: none;" id="addrglup">
   ...
   <div class="form-group">
    <label for="addr3" class="col-sm-2 control-label" id="addrtitle">收货地址</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="addr3" name="recipients2" placeholder="" value="<?php echo ($addr); ?>" style="width: 75%;margin-right: 3%;float: left;" disabled="disable">
...
<div class="group">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" id="name2" name="receivename" placeholder="姓名" value="<?php echo ($info['receiver']); ?>">
    </div>
  </div>
  <div class="form-group">
    <label for="tel2" class="col-sm-2 control-label"><span class="payattention">*</span>手机/固话</label>
    <div class="col-sm-10">
      <input type="tel" class="form-control" id="tel2" placeholder="手机号/固话号" name="receivephone" value="<?php echo ($info['receiving_phone']); ?>">
    </div>
  </div>
  <div class="form-group">
    <label for="unit2" class="col-sm-2 control-label">收件单位</label>
    <div class="col-sm-10">
      <input type="tel" class="form-control" id="unit2" placeholder="收件单位" name="cunit" value="<?php echo ($info['receiving_company']); ?>">
    </div>
  </div>
  <div class="form-group" style="display: none;" id="shoujian">
  ...
  <div class="form-group">
    <label for="addr2" class="col-sm-2 control-label"><span class="payattention" id="shoujian1">*</span>收件地址</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="addr2" disabled="disabled" name="sendaddr" placeholder="请输入详细收货地址" style="width: 75%;margin-right: 3%;float: left;" value="<?php echo ($info['receiving_address']); ?>">
      <button type="button" class="btn btn-primary btn-lg" id="changeaddr1" style="width: 22%;height: 70%;font-size: 1em;padding: 7px;">
 更换地址
</button>
    </div>
  </div>
...
<div class="group">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
     ...
     <?php if(is_array($expressname)): $i = 0; $__LIST__ = $expressname;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$express): $mod = ($i % 2 );++$i;?><option value="<?php echo ($express['id']); ?>" <?php if($express['name']==$info['name']){echo "selected=selected";}?> ><?php echo ($express['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
    </div>
  </div>
 ...
 <div class="form-group">
    ...
    <div class="col-sm-10">
      <input type="number" class="form-control" placeholder="预计重量（kg）" id="weight" name="weight" value="<?php echo ($info['sender_estimated_weight']); ?>">
    </div>
  </div>
   <div class="form-group">
    <label for="option3" class="col-sm-2 control-label"><span class="payattention">*</span>取件备注</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" placeholder="易碎品、液体、贵重物品请特殊说明" id="option3" name="remark2" value="<?php echo ($info['express_remarks']); ?>">
    </div>
  </div>
  ...
  <input type="submit" class="btn btn-primary" value="立即下单" name="submit" id="submit" ><?php endif; ?>
    ...
    var strSession = "<?php echo $_SESSION['name']; ?>".toString();
      ...
      $('#changeaddr').click(function(){
        ...
        $("#addr3").val('');
        $('#addr3').attr('name','recipients1');
        $('#addr3').removeAttr("disabled");
        $("#addrtitle").html(''); 
      });
      $('#changeaddr1').click(function(){
        $("#shoujian").css({
          "display":"block"
        });
        $("#addr2").val('');
        $("#addr2").attr('name','sender1');
