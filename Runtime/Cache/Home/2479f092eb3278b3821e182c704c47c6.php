...
<div class="container-totton-con" style="min-height: 400px;">
			...
			<div class="bottontitle"><span class="bottontitle-span1">管理上门寄件地址</span><a href="<?php echo U('index/addaddr',array('flag'=>1));?>" style="margin-left: 20px;"><button type="button" class="btn btn-info"">新增地址</button></a></div>
  		...
  		<div class="information" style="text-align: center;margin-top: 15px;">
			<table class="table table-striped table-bordered" style="width: 80%;margin:0 auto;text-align: center;">
				...
				<?php if(is_array($addr)): $i = 0; $__LIST__ = $addr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$add): $mod = ($i % 2 );++$i;?><tr>
					...
					<td style="text-align: left;"><a href="<?php echo U('index/setaddr',array('id'=>$add['id'],'flag'=>1));?>" ">设为默认地址</a><a href="<?php echo U('index/changeaddr',array('id'=>$add['id'],'flag'=>1));?>">编辑</a><?php if(($add['sign'] == 0)): ?><a href="<?php echo U('index/deleteaddr',array('id'=>$add['id'],'flag'=>1));?>">删除</a><?php endif; ?></td>
