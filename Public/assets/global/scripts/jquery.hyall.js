...
jQuery.fn.hyall = function(config){
     ...
     * 表单生成器
     */ 
    //formBuilder(the.config.columns, false, {_submit:'insert'}, 'add');
    this.formBuilder = function(columns, val, hidden, type){
    	...
    	$.each(hidden, function(k, v){
    		html+='<input type="hidden" name="'+k+'" value="'+v+'">';//装填token
    	});
    	var rules = {};
    	var _type = type;
    ...
    //formBuilder(the.config.columns, false, {_submit:'insert'}, 'add');
    this.scanAddFormBuilder = function(columns, val, hidden, type){
        var html = '';
        if(!val) val = false;
    ...
    this.searchChangeFormBuilder = function(columns, val, hidden, type){
        var html = '';
        if(!val) val = false;
        ...
        $.each(columns, function(k, v){
            ...
            var title=(v.form.title || v.title);
            if(!title) return true;
            if(!v.form.searchChange) return true;
            if(!v.form.validate) v['form']['validate']=false;
            else rules[('select'===v.form.type && v.form.select && v.form.select.multiple)?k+'[]':k]=v.form.validate;
            html+='<div class="form-group"><label class="control-label col-md-3 col-sm-10" for="'+k+'">'+title;
            html+=(v.form.validate.required ? '<span class="required">&nbsp;*</span>' : '&nbsp;&nbsp;')+ '</label><div class="col-md-6 col-sm-10">';
    ...
    this.actionsHandlers = {
    	formSubmit:function($form,onComplete){
            ...
            //获取name=_submit这个隐藏的input元素获得这个表单的提交类型
            /*/Project/Marxism/marks/index.php/Achieve/Achiever/ajax?q=inserts*/
        ...
        initSearchChange: function($modal){
            // console.log(the.config.columns);
            var build=the.searchChangeFormBuilder(the.config.columns, false, {_submit:'searchChange'}, 'change');
  
            var tpl={
                    modal: 'change',
                    size: $modal.data('size') || '',
                    title:{
                        icon:the.config.options.buttons.change.icon,
                        action:the.config.options.buttons.change.title
                    },
                    body:{
                        main:build.html
                    }
            ...
            the.setModal(tpl);
            if(the.config.options.tips.change) $modal.find('.alert-tips').show().find('span').html(the.config.options.tips.change);
            the.validateFormHandler({form: $modal.find('form'), rules: build.rules, hy: true});
            $modal.on('shown.bs.modal',function(){
                // the.trigger('shown.hyall.form.add');
            });
        ...
        initReceiptChange:function($modal,title,ajaxName,modalName){
             // console.log(the.config.columns);
            var build=the.searchChangeFormBuilder(the.config.columns, false, {_submit:ajaxName}, 'change');
    	...
    	initScanAdd:function($modal,title,ajaxName,modalName){
             // console.log(the.config.columns);
            var build=the.scanAddFormBuilder(the.config.columns, false, {_submit:ajaxName}, 'scanAdd');
    	...
    	actionEdit: function(rows, $modal, columns){
    	    ...
    	    $.each(rows,function(k,v){
    	    	$.post($.U('ajax?q=edit'), {pk:v}, function(r){
	        	    ...
	        	    if(the.config.options.tips.edit) 
                        $modal.find('.alert-tips').show().find('span').html(the.config.options.tips.edit);
	        		$modal.on('shown.bs.modal',function(){
        ...
        actionBind: function(rows, $modal, columns){
            $.loading();
            $.each(rows,function(k,v){
                $.post($.U('ajax?q=edit'), {pk:v}, function(r){
                    $.unloading();
                    ...
                    if($.isPlainObject(r.columns)) columns=r.columns;
                    var build=the.searchChangeFormBuilder(columns,r.val,{_submit:'update',_token:r.token},'bind');
                    var tpl={
                            modal:'bind',
                            size: $modal.data('size') || '',
                            title:{
                    ...
                    $modal.on('shown.bs.modal',function(){
                        the.trigger('shown.hyall.form.bind');
                    });
                    $modal.modal('toggle');
                });
            });
	...
	this.init = function(){
        ...
        dt.init($.extend(true, {
            ...
            dataTable: $.extend({
                ...
                ajax: {
                    // 使用ajax从数据库初始化数据，在datatable初始化的时候执行
                	url: $.U('ajax?q=list')
                },
                columns: dtColumns
            }, tools ? tools : {})
		...
		if($.isPlainObject(config.options.buttons)){
			...
			if(config.options.buttons.add)
                $('[data-action="add"]', $ctx).on('click',function(e){
                    e.stopPropagation();
    				the.actionsHandlers.initAdd($modal);
                    //return;
    			});
            if(config.options.buttons.change) 
                $('[data-action="change"]', $ctx).on('click',function(e){
                    e.stopPropagation();
                    the.actionsHandlers.initSearchChange($modal);
                    //return;
                });

            if(config.options.buttons.receipt_change) 
                $('[data-action="receipt_change"]', $ctx).on('click',function(e){
                    e.stopPropagation();
                    var title = {
                        icon:the.config.options.buttons.receipt_change.icon,
                        action:the.config.options.buttons.receipt_change.title
                    };
                    the.actionsHandlers.initReceiptChange($modal,title,'receipt','receiptChange');
                    //return;
                });
            if(config.options.buttons.unreceipt_change) 
                $('[data-action="unreceipt_change"]', $ctx).on('click',function(e){
                    e.stopPropagation();
                    var title = {
                        icon:the.config.options.buttons.unreceipt_change.icon,
                        action:the.config.options.buttons.unreceipt_change.title
                    };
                    the.actionsHandlers.initReceiptChange($modal,title,'unreceipt','unreceiptChange');
                    //return;
                });

            if(config.options.buttons.scan_add) 
                $('[data-action="scan_add"]', $ctx).on('click',function(e){
                    e.stopPropagation();
                    var title = {
                        icon:the.config.options.buttons.scan_add.icon,
                        action:the.config.options.buttons.scan_add.title
                    };
                    the.actionsHandlers.initScanAdd($modal,title,'scanAdd','scanAdd');
                    //return;
                });
