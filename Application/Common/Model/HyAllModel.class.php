...
abstract class HyAllModel  extends HyFrameModel{
	 ...
	 *@param array $json 要返回的数据，引用型传递
	 */
	protected function ajax_searchChange(&$json){
		$Number = trim(I('courier_number'));
		//dump(preg_match('/[0-9A-Za-z]{8,16}/',$Number));
		if(!preg_match('/[0-9A-Za-z]{8,16}/',$Number)){
			$json['info'] = '快递单号不合法！';
			return;
		}
		$this->updateOrderStatus($json, $Number);
	}
	/**
	 * 订单出库
	 *
	 *@param 快递单号
	 */
	protected function ajax_judgeCanChange(&$json){
		$number = trim(I('courier_number'));
		...
		$super = M('frame_role')->max('id');
		$userid = ss_uid();
		$user = M('user')->where(array('id' => $userid))->find();
		if(session('roleId') == $super){
			$value = $this->where(array('courier_number'=>array('eq',$number)))->find('hy');
		}else{
			$value = $this->where(array(
				'courier_number'=>array('eq',$number),
				'dot'=>array('eq',$user['area_id'])))->find('hy');
		}
		//dump($Number);
	...
	protected function updateOrderStatus(&$json, $number){
		...
		$this->save($value);
		$log = array('msg'=>"快递单号为{$Number}的订单出库");
		Hook::listen('hy_log', $log);
		$json['status'] = true;
		$json['info'] = ($json['status'] ? '出库成功！' : ($this->getError() ?: '出库失败！'));
	...
	protected function getDtData(){
		...
		//=============<PC CODE BEGIN>=============
		if(CONTROLLER_NAME == 'SendOrder'){
			$lists[] = SendOrder_total_money($lists);
		}
		//=============<PC CODE END>===============
		$records["data"] = $lists;
	...
	protected function write($type='', $pk=null){
		...
		if($data['weight'] != null && $data['weight'] != 0){
			$arrr = M('express_price')->where(array(
    		'area_id' => $data['price_area'],
    		'express_id' => $data['express_id'],
    		'status' => array('lt',9)
    		))->select();
	    	if($data['weight'] <= $arrr[0]['first_weight'] && $data['weight'] > 0){
	    		$userPrice = $arrr[0]['first_price'];
	    		$expressPrice = $arrr[0]['express_first_price'];
	    	}else if($data['weight'] > $arrr[0]['first_weight']){
	    		$userPrice = $arrr[0]['first_price']+($data['weight']-$arrr[0]['first_weight'])*$arrr[0]['additional_price'];
	    		$expressPrice = $arrr[0]['express_first_price']+($data['weight']-$arrr[0]['first_weight'])*$arrr[0]['express_additional_price'];
	    	...
	    	$data['need_payment'] = $userPrice;
	    	$data['express_payment'] = $expressPrice;
		}
	...
	protected function searchMap() {
		...
		foreach ($search as $k => $v ) {
			...
			if($tmp = $this->fieldsOptions[$k]['list']['search']['sql']){
				$mapKey[$k] = true;
				$tempTimeStamp = $this->fieldsOptions[$k]['list']['search']['timestamp']? true:false;
				$tmp=preg_replace_callback('/\{:([\w\-#]+)}/', function ($matches) use ($search, $tempTimeStamp){
					if($tempTimeStamp)
						$search[$matches[1]] = strtotime($search[$matches[1]]);
					else;
